...
export const Inventory2ndGrade: React.FC = () => {
    ...
    const handleDownloadPdf = () => {
        ...
        classrooms.forEach((cls, index) => {
            ...
            doc.setFontSize(12);
            doc.text(`Grade 2 Inventory`, 14, 28);
        ...
        if (classrooms.length > 0) {
            doc.addPage();
            doc.setFontSize(16);
            doc.text(`R/EMB/ROYAL COLLEGE - Grade 2 Total Summary`, 14, 20);
        ...
        doc.save('grade_2_classroom_inventory.pdf');
    };

    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Manage Classroom Inventory (Grade 2)</h3>
            ...
            <form onSubmit={handleAddClassroom} className="flex items-end gap-4 p-4 bg-slate-50 rounded-lg">
                <div className="flex-grow">
                    <label htmlFor="newClassNameGrade2" className="block text-sm font-medium text-slate-600 mb-1">New Class Room Name</label>
                    <input id="newClassNameGrade2" type="text" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} placeholder="e.g., 2D" className="w-full px-3 py-2 border border-slate-300 rounded-lg" required />
                </div>
