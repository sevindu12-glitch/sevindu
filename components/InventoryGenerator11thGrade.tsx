...
export const InventoryGenerator11thGrade: React.FC = () => {
    const [classroomNames, setClassroomNames] = useState('11A, 11B, 11C, 11D, 11E');
    ...
    const handleDownloadPdf = () => {
        ...
        const title = "Grade 11 Summary Report";
        ...
        doc.save(`grade_11_inventory_summary.pdf`);
    };

    return (
        <div className="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Create Inventory PDF (Grade 11)</h3>
            
            <div className="mb-4">
                <label htmlFor="classroomNamesGrade11" className="block text-sm font-medium text-slate-600 mb-1">
                    Classroom Names (comma-separated)
                </label>
                <textarea
                    id="classroomNamesGrade11"
                    value={classroomNames}
                    onChange={(e) => setClassroomNames(e.target.value)}
                    placeholder="e.g., 11A, 11B, 11C"
            ...
            <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end p-4 bg-slate-50 rounded-lg">
                <div className="md:col-span-1">
                    <label htmlFor="newItemNameGrade11" className="block text-sm font-medium text-slate-600 mb-1">Item Name</label>
                    <input
                        id="newItemNameGrade11"
                        type="text"
                        value={newItemName}
                 ...
                 <div className="md:col-span-1">
                    <label htmlFor="newItemQuantityGrade11" className="block text-sm font-medium text-slate-600 mb-1">Quantity</label>
                    <input
                        id="newItemQuantityGrade11"
                        type="number"
