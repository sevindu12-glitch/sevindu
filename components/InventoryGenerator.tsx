...
export const InventoryGenerator: React.FC = () => {
    const [classroomNames, setClassroomNames] = useState('6A, 6B, 6C, 6D, 6E');
    ...
    const handleDownloadPdf = () => {
        ...
        const title = "Grade 6 Summary Report";
        ...
        doc.save(`grade_6_inventory_summary.pdf`);
    };

    return (
        <div className="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Create Inventory PDF (Grade 6)</h3>
            
            <div className="mb-4">
                <label htmlFor="classroomNamesGrade6" className="block text-sm font-medium text-slate-600 mb-1">
                    Classroom Names (comma-separated)
                </label>
                <textarea
                    id="classroomNamesGrade6"
                    value={classroomNames}
                    onChange={(e) => setClassroomNames(e.target.value)}
                    placeholder="e.g., 6A, 6B, 6C"
            ...
            <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end p-4 bg-slate-50 rounded-lg">
                <div className="md:col-span-1">
                    <label htmlFor="newItemNameGrade6" className="block text-sm font-medium text-slate-600 mb-1">Item Name</label>
                    <input
                        id="newItemNameGrade6"
                        type="text"
                        value={newItemName}
                 ...
                 <div className="md:col-span-1">
                    <label htmlFor="newItemQuantityGrade6" className="block text-sm font-medium text-slate-600 mb-1">Quantity</label>
                    <input
                        id="newItemQuantityGrade6"
                        type="number"
