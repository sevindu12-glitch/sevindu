...
const defaultGrade1Inventory = { 
    ...
    fans: {usable: 2, broken: 0}, 
    plugTops: {usable: 2, broken: 0}, 
...
const pichchaInventory = {
    ...defaultGrade1Inventory,
    childChairs: { usable: 38, broken: 2 },
    childDesks: { usable: 38, broken: 1 },
    fans: { usable: 2, broken: 0 },
...
const roseInventory = {
    ...defaultGrade1Inventory,
    childChairs: { usable: 39, broken: 0 },
    childDesks: { usable: 39, broken: 0 },
    fans: { usable: 2, broken: 0 },
    plugTops: { usable: 1, broken: 0 },
    waterFilters: { usable: 0, broken: 0 },
    lightbulbsAndHolders: { usable: 2, broken: 0 },
...
const initialClassrooms: Classroom[] = [
    ...
    { id: '2', name: 'ROSE', items: getInitialItems(roseInventory)},
    { id: '3', name: 'IDDA', items: getInitialItems(iddaInventory)},
    { id: '4', name: 'PICHCHA', items: getInitialItems(pichchaInventory)},
];
...
export const Inventory1stGrade: React.FC = () => {
    ...
    const handleDownloadPdf = () => {
        ...
        classrooms.forEach((cls, index) => {
            ...
            doc.setFontSize(12);
            doc.text(`Grade 1 Inventory`, 14, 28);
        ...
        if (classrooms.length > 0) {
            doc.addPage();
            doc.setFontSize(16);
            doc.text(`R/EMB/ROYAL COLLEGE - Grade 1 Total Summary`, 14, 20);
        ...
        doc.save('grade_1_classroom_inventory.pdf');
    };

    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Manage Classroom Inventory (Grade 1)</h3>
            ...
            <form onSubmit={handleAddClassroom} className="flex items-end gap-4 p-4 bg-slate-50 rounded-lg">
                <div className="flex-grow">
                    <label htmlFor="newClassNameGrade1" className="block text-sm font-medium text-slate-600 mb-1">New Class Room Name</label>
                    <input id="newClassNameGrade1" type="text" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} placeholder="e.g., 1D" className="w-full px-3 py-2 border border-slate-300 rounded-lg" required />
                </div>
