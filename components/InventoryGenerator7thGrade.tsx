...
export const InventoryGenerator7thGrade: React.FC = () => {
    const [classroomNames, setClassroomNames] = useState('7A, 7B, 7C, 7D, 7E');
    ...
    const handleDownloadPdf = () => {
        ...
        const title = "Grade 7 Summary Report";
        ...
        doc.save(`grade_7_inventory_summary.pdf`);
    };

    return (
        <div className="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Create Inventory PDF (Grade 7)</h3>
            
            <div className="mb-4">
                <label htmlFor="classroomNamesGrade7" className="block text-sm font-medium text-slate-600 mb-1">
                    Classroom Names (comma-separated)
                </label>
                <textarea
                    id="classroomNamesGrade7"
                    value={classroomNames}
                    onChange={(e) => setClassroomNames(e.target.value)}
                    placeholder="e.g., 7A, 7B, 7C"
            ...
            <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end p-4 bg-slate-50 rounded-lg">
                <div className="md:col-span-1">
                    <label htmlFor="newItemNameGrade7" className="block text-sm font-medium text-slate-600 mb-1">Item Name</label>
                    <input
                        id="newItemNameGrade7"
                        type="text"
                        value={newItemName}
                 ...
                 <div className="md:col-span-1">
                    <label htmlFor="newItemQuantityGrade7" className="block text-sm font-medium text-slate-600 mb-1">Quantity</label>
                    <input
                        id="newItemQuantityGrade7"
                        type="number"
