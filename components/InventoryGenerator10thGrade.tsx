...
export const InventoryGenerator10thGrade: React.FC = () => {
    const [classroomNames, setClassroomNames] = useState('10A, 10B, 10C, 10D, 10E');
    ...
    const handleDownloadPdf = () => {
        ...
        const title = "Grade 10 Summary Report";
        ...
        doc.save(`grade_10_inventory_summary.pdf`);
    };

    return (
        <div className="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Create Inventory PDF (Grade 10)</h3>
            
            <div className="mb-4">
                <label htmlFor="classroomNamesGrade10" className="block text-sm font-medium text-slate-600 mb-1">
                    Classroom Names (comma-separated)
                </label>
                <textarea
                    id="classroomNamesGrade10"
                    value={classroomNames}
                    onChange={(e) => setClassroomNames(e.target.value)}
                    placeholder="e.g., 10A, 10B, 10C"
            ...
            <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end p-4 bg-slate-50 rounded-lg">
                <div className="md:col-span-1">
                    <label htmlFor="newItemNameGrade10" className="block text-sm font-medium text-slate-600 mb-1">Item Name</label>
                    <input
                        id="newItemNameGrade10"
                        type="text"
                        value={newItemName}
                 ...
                 <div className="md:col-span-1">
                    <label htmlFor="newItemQuantityGrade10" className="block text-sm font-medium text-slate-600 mb-1">Quantity</label>
                    <input
                        id="newItemQuantityGrade10"
                        type="number"
