...
const initialClassrooms: Classroom[] = [
    { id: '1', name: 'Kumudu', items: getInitialItems(defaultPrimaryInventory)},
    ...
    { id: '5', name: 'Deliya', items: getInitialItems(defaultPrimaryInventory)},
];
...
export const Inventory5thGrade: React.FC = () => {
    ...
    const handleDownloadPdf = () => {
        ...
        classrooms.forEach((cls, index) => {
            ...
            doc.setFontSize(12);
            doc.text(`Grade 5 Inventory`, 14, 28);
        ...
        if (classrooms.length > 0) {
            doc.addPage();
            doc.setFontSize(16);
            doc.text(`R/EMB/ROYAL COLLEGE - Grade 5 Total Summary`, 14, 20);
        ...
        doc.save('grade_5_classroom_inventory.pdf');
    };

    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Manage Classroom Inventory (Grade 5)</h3>
            ...
            <form onSubmit={handleAddClassroom} className="flex items-end gap-4 p-4 bg-slate-50 rounded-lg">
                <div className="flex-grow">
                    <label htmlFor="newClassNameGrade5" className="block text-sm font-medium text-slate-600 mb-1">New Class Room Name</label>
                    <input id="newClassNameGrade5" type="text" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} placeholder="e.g., 5D" className="w-full px-3 py-2 border border-slate-300 rounded-lg" required />
                </div>
