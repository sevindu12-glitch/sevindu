...
const defaultPrimaryInventory = { 
    ...
    whiteCubicles: {usable: 1, broken: 0}, 
...
const kumuduIddaInventory = {
    ...defaultPrimaryInventory,
    childChairs: { usable: 41, broken: 1 },
    childDesks: { usable: 42, broken: 0 },
    teacherDesks: { usable: 1, broken: 0 },
    teacherChairs: { usable: 1, broken: 0 },
    whiteboards: { usable: 2, broken: 1 },
    blackboards: { usable: 0, broken: 0 },
    cupboards: { usable: 1, broken: 0 },
    fans: { usable: 3, broken: 2 },
    plugTops: { usable: 1, broken: 0 },
...
const initialClassrooms: Classroom[] = [
    { id: '1', name: 'Kumudu', items: getInitialItems(kumuduIddaInventory)},
    { id: '2', name: 'ROSE', items: getInitialItems(defaultPrimaryInventory)},
    { id: '3', name: 'IDDA', items: getInitialItems(kumuduIddaInventory)},
...
export const Inventory4thGrade: React.FC = () => {
    ...
    const handleDownloadPdf = () => {
        ...
        classrooms.forEach((cls, index) => {
            ...
            doc.setFontSize(12);
            doc.text(`Grade 4 Inventory`, 14, 28);
        ...
        if (classrooms.length > 0) {
            doc.addPage();
            doc.setFontSize(16);
            doc.text(`R/EMB/ROYAL COLLEGE - Grade 4 Total Summary`, 14, 20);
        ...
        doc.save('grade_4_classroom_inventory.pdf');
    };

    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl md:text-2xl font-bold text-slate-700 mb-4 border-b pb-3">Manage Classroom Inventory (Grade 4)</h3>
            ...
            <form onSubmit={handleAddClassroom} className="flex items-end gap-4 p-4 bg-slate-50 rounded-lg">
                <div className="flex-grow">
                    <label htmlFor="newClassNameGrade4" className="block text-sm font-medium text-slate-600 mb-1">New Class Room Name</label>
                    <input id="newClassNameGrade4" type="text" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} placeholder="e.g., 4D" className="w-full px-3 py-2 border border-slate-300 rounded-lg" required />
                </div>
